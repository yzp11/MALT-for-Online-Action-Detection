        ...
        for phase in cfg.SOLVER.PHASES:
            ...
            with torch.set_grad_enabled(training):
                ...
                for batch_idx, data in enumerate(pbar, start=1):
                    ...
                    det_score, enc1_score, enc2_score = model(*[x.to(device) for x in data[:-1]])
                    det_score = det_score.reshape(-1, cfg.DATA.NUM_CLASSES)
                    enc1_score = enc1_score.reshape(-1, cfg.DATA.NUM_CLASSES)
                    enc2_score = enc2_score.reshape(-1, cfg.DATA.NUM_CLASSES)

                    det_target = det_target.reshape(-1, cfg.DATA.NUM_CLASSES)
                    det_loss = criterion['MCE'](det_score, det_target)
                    aux1_loss = criterion['MCE'](enc1_score, det_target)
                    aux2_loss = criterion['MCE'](enc2_score, det_target)
                    det_loss=det_loss + aux1_loss*0.4+ aux2_loss*0.4
