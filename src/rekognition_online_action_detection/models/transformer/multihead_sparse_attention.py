...
class DotProductSparseAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None,num_k=370):
...
class DotProductSparseAttentionStream(DotProductSparseAttention):

    def __init__(self, dropout=0.0):
        super(DotProductSparseAttentionStream, self).__init__(dropout)
...
class MultiheadSparseAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        ...
        self.dotproductattention = DotProductSparseAttention(dropout)

    def forward(self, q, k, v, attn_mask=None, key_padding_mask=None,num_k=370):
        ...
        attn_output = self.dotproductattention(q, k, v, mask,num_k)
        attn_output = attn_output.transpose(0, 1).contiguous().view(tsz, bsz,
                                                                    self.embed_dim)
        return self.out_proj(attn_output), None


class MultiheadSparseAttentionStream(MultiheadSparseAttention):

    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        super(MultiheadSparseAttentionStream, self).__init__(embed_dim, num_heads, dropout, bias, kdim, vdim)

        self.dotproductattention = DotProductSparseAttentionStream(dropout)
