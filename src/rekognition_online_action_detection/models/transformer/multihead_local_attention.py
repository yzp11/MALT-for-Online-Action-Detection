...
class DotProductLocalAttention(nn.Module):

    def __init__(self, dropout=0.0):
        super(DotProductLocalAttention, self).__init__()

        self.dropout = dropout

    def forward(self, query, key, value, attn_mask=None, window_size=5):
        ...
        # Apply a mask to limit the attention to a local window.
        # real window=2*window_size
        mask = torch.zeros((batch_size, query_len, key_len),device=query.device)
        for i in range(query_len):
            mask[:, i, max(0, i - window_size):min(key_len, i + window_size)] = 1
...
class DotProductLocalAttentionStream(DotProductLocalAttention):

    def __init__(self, dropout=0.0):
        super(DotProductLocalAttentionStream, self).__init__(dropout)
...
class MultiheadLocalAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        ...
        self.dotproductlocalattention = DotProductLocalAttention(dropout)
    ...
    def forward(self, q, k, v, attn_mask=None, key_padding_mask=None):
        ...
        attn_output = self.dotproductlocalattention(q, k, v, mask)
        attn_output = attn_output.transpose(0, 1).contiguous().view(tsz, bsz,
                                                                    self.embed_dim)
        return self.out_proj(attn_output), None


class MultiheadLocalAttentionStream(MultiheadLocalAttention):

    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        super(MultiheadLocalAttentionStream, self).__init__(embed_dim, num_heads, dropout, bias, kdim, vdim)

        self.dotproductlocalattention = DotProductLocalAttentionStream(dropout)
    ...
    def stream_inference(self, q, k, v, pos, attn_mask=None, key_padding_mask=None):
        ...
        attn_output = self.dotproductlocalattention.stream_inference(q, k, v, k_pos, v_pos, mask)
        attn_output = attn_output.transpose(0, 1).contiguous().view(tsz, bsz,
                                                                    self.embed_dim)
        return self.out_proj(attn_output), None
